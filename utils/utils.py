...
def compute_loss(p, targets, m, m_targets, alpha, model):  # predictions, targets, model
    ...
    # for i, out in enumerate(p):
    #     print(f"p[{i}] device: {out.device}")
    ...
    if not torch.isfinite(m).all():
        # print("Non-finite values in `m`:", m)
    ...
    #ldepth += (1 - ssim(m.float().cuda(), m_targets.float().cuda()))
...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate([model.yolo1, model.yolo2, model.yolo3]):
        ...
        # print(f"gain device: {gain.device}")
        # print(f"t device: {t.device}")
        # for i, out in enumerate(a):
        #     print(f"a[{i}] device: {out.device}")
        # print("a:", a)
        gwh = t[:, 4:6]
        if nt:
