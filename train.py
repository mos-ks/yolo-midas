...
conf_path = os.path.join(current_directory, "cfg", "mde.cfg")
conf.read(conf_path)
yolo_props = {}
...
def train():
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, dp_imgs, pln_imgs) in pbar:  # batch -------------------------------------------------------------
            ...
            # print(f"midas_out device: {midas_out.device}")
            # print(f"dp_imgs device: {dp_imgs.device}")
            # for i, out in enumerate(yolo_out):
            #     print(f"yolo_out[{i}] device: {out.device}")
            # print(kls)
    
            if not torch.isfinite(midas_out).all(): print("Problematic depth output: ", i)
            # Compute loss
